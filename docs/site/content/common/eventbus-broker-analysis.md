---
provenance: |
  Documento consolidado en la seccion "Common". El mismo analisis existia
  duplicado, byte por byte (md5 5fe13714947fcbb60e7ce68248dca9eb), en los
  directorios docs/architecture/ de cinco servicios: agentic-core,
  compliance-core, invoice-core, marketplace-core y payments-core.
  Esta es la copia canonica unica.
---

# Decisiones de Arquitectura del Bus de Eventos: Análisis de Implicaciones

!!! info "Documento Common (consolidado)"
    Este analisis del bus de eventos es **material transversal**: describe el
    broker que une a `marketplace-core`, `payments-core`, `invoice-core` y
    `compliance-core` (mas integraciones con pasarelas y apps Dart). Existia
    duplicado identico (md5 `5fe13714947fcbb60e7ce68248dca9eb`) en
    `services/{agentic,compliance,invoice,marketplace,payments}-core/docs/architecture/eventbus-broker-analysis.md`.
    Se consolida aqui una sola vez con procedencia explicita para evitar divergencia.
